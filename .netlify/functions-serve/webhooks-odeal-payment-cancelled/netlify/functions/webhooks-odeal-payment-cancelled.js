...
// netlify/functions/webhooks-odeal-payment-cancelled.js
var webhooks_odeal_payment_cancelled_exports = {};
__export(webhooks_odeal_payment_cancelled_exports, {
  handler: () => handler
});
module.exports = __toCommonJS(webhooks_odeal_payment_cancelled_exports);

// api/_lib/verify.js
function verifyOdeal(req, res) {
  const expected = process.env.ODEAL_REQUEST_KEY;
  const got = req.headers["x-odeal-request-key"] || req.headers["X-ODEAL-REQUEST-KEY"] || req.headers["X-Odeal-Request-Key"];
...
// src/ropClient.js
var ROP_BASE_URL = process.env.ROP_BASE_URL || "http://test.ropapi.com/V6/App2App";
...
function makeHttp() {
  const isHttps2 = /^https:\/\//i.test(ROP_BASE_URL);
  if (process.env.NODE_ENV === "production" && !isHttps2) {
  ...
  const instance = axios_default.create({
    timeout: Number(process.env.ROP_HTTP_TIMEOUT_MS || 5e3),
    maxRedirects: 0,
...
async function postPaymentStatus({
  ...
  const base = reqBase();
  const url2 = `${ROP_BASE_URL}/PaymentStatus`;
  const body = {
    ...base,
    ...
    Invoice: Invoice || void 0
  };
  const { data } = await http2.post(url2, body);
  return data;
}

// src/basketProvider.js
var PROVIDER = (process.env.BASKET_PROVIDER || "mock").toLowerCase();
var DEFAULT_TOTAL = Number(process.env.BASKET_DEFAULT_TOTAL || "100.00");
...
var TTL_MS = Number(process.env.IDEMPOTENCY_TTL_MS || 10 * 60 * 1e3);
var MAX_KEYS = Number(process.env.IDEMPOTENCY_MAX_KEYS || 1e4);
...
async function handler(event, context) {
  ...
  try {
    ...
    await handleWebhook("payment-cancelled", req, res);
    return {
      statusCode: res.statusCode,
      headers: res.headers,
      body: res.body
...
mime-types/index.js:
   ...
   *)
*/
//# sourceMappingURL=webhooks-odeal-payment-cancelled.js.map
